...
MODEL_STATE_FILE = path.join(path.dirname(path.abspath(__file__)), "model_state_6.pth")
...
class GAT(nn.Module):
    ...
    def __init__(self, g, n_layers, input_size, hidden_size, output_size, nonlinearity, num_heads):
        super().__init__()

        self.g = g
        self.layers = nn.ModuleList()
        self.layers.append(GATConv(input_size, hidden_size, activation=nonlinearity, num_heads=num_heads))
        for i in range(n_layers - 1):
            self.layers.append(GATConv(hidden_size, hidden_size, activation=nonlinearity, num_heads=num_heads))
        self.layers.append(GATConv(hidden_size, output_size, num_heads=num_heads))
        print(self.layers)
...
def main(args):
    ...
    model = GAT(g=train_dataset.graph, n_layers=6, input_size=n_features, hidden_size=512,
                output_size=n_classes, nonlinearity=F.elu, num_heads=1).to(device)
